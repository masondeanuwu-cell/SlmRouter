...
import type { ProxyConfig, InsertProxyConfig } from "@shared/schema";
...
export default function ProxyConfig() {
  ...
  const createConfigMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({
        title: "Proxy Configuration Updated",
    ...
    <div className="bg-white rounded-xl border border-slate-200 p-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        >
          ...
          <span>{createConfigMutation.isPending ? "Updating..." : "Start Proxy"}</span>
        </button>
      </form>
      ...
      {currentConfig && (
        <div className="mt-4 p-3 bg-slate-50 rounded-md">
          <p className="text-xs text-slate-600">
            Current: <span className="font-mono">{currentConfig.targetUrl}</span>
          </p>
        </div>
      )}
    </div>
